...
export interface MagicLinkEmailData {
  ...
  documents: Array<{
    ...
    buffer?: Buffer;     // File buffer for Vercel (from Supabase Storage)
  }>;
...
export const sendMagicLinkEmail = async (data: MagicLinkEmailData): Promise<void> => {
  ...
  // Process document attachments (using buffers from Supabase Storage)
  for (const doc of data.documents) {
    if (doc.buffer && doc.fileName) {
      try {
  ...
  const documentLinks = data.documents.map(doc => {
    const downloadUrl = `${baseUrl}/api/access/${data.magicLinkToken}/download/${doc.id}`;
    return `
      <tr>
