...
/**
 * Validates and processes attachments - NOTE: For Vercel, attachments should come from Supabase Storage
 */
function processAttachments(attachments?: EmailAttachment[]): {
  ...
  for (const attachment of attachments) {
    try {
      ...
      // For Next.js/Vercel: Only support buffer content (no filesystem paths)
      if (attachment.content) {
        content = attachment.content;
        fileSize = content.length;
      } else {
        errors.push(`${attachment.filename}: No buffer content provided (filesystem paths not supported on Vercel)`);
        continue;
      }
      ...
      errors.push(`${attachment.filename}: ${error.message || 'Processing error'}`);
    }
  }
